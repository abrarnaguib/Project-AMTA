...
// Order Parsers for database (.tsv based)
std::string Order::Serialize() const {
    std::ostringstream oss;
    oss << m_orderId << "\t" << m_retailerId << "\t" << m_dealerId << "\t" << m_productId << "\t" << m_quantity << "\t" << OrderStatusToString(m_status);
    return oss.str();
}
...
std::string Order::ToString() const {
    std::ostringstream oss;
    oss << "Order[" << m_orderId << "]" << " Product:" << m_productId << " Qty:" << m_quantity << " Status:" << OrderStatusToString(m_status);
    return oss.str();
}
